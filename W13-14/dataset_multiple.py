...
class SceneTextDataset(Dataset):
    def __init__(self, root_dir, split='train', image_size=1024, crop_size=512, color_jitter=True,
        ...
        logging.info(f'load annotation: {split[0]}.json')
        with open(osp.join(root_dir, 'ufo/{}.json'.format(split[0])), 'r') as f:
            anno = json.load(f)
        
        if len(split) > 1:
            for split_name in split[1:]:
                with open(osp.join(root_dir, 'ufo/{}.json'.format(split_name)), 'r') as f:
                    anno_additional = json.load(f)
                logging.info(f'update annotation: {split_name}.json')
                anno['images'].update(anno_additional['images'])
...
if __name__ == '__main__':
    ...
    # ds = SceneTextDataset('/opt/ml/input/data/upstage_dataset_resize', 'train_rect')
    # print('# of samples:', len(ds))
    # print('image size:', ds[0][0].shape)

    ds0 = SceneTextDataset('/opt/ml/input/data/ICDAR17_LKJ_resize', 'train')
    print('# of samples:', len(ds0))
    print('image size:', ds0[0][0].shape)
    ds1 = SceneTextDataset('/opt/ml/input/data/ICDAR19_LKJ_resize', 'train')
    print('# of samples:', len(ds1))
    print('image size:', ds1[0][0].shape)
    ds2 = SceneTextDataset('/opt/ml/input/data/upstage_dataset_resize', 'train_rect')
    print('# of samples:', len(ds2))
    print('image size:', ds2[0][0].shape)

    ds_concat = torch.utils.data.ConcatDataset([ds0, ds1, ds2])
    print('# of samples:', len(ds_concat))