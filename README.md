...
- [One-Stage Detectors](https://github.com/star-bits/boostcampAI/blob/main/W10-12/%EC%A0%95%EB%A6%AC_1_Stage_Detectors.ipynb): One-stage detectors have no RPN, but just conv layers. YOLO (conv output layer with 30 channels, each representing grid, bbox, class, etc.; loss: localization loss + confidence loss + classification loss), SSD(multi-scale feature maps, no fc layer, anchor box), RetinaNet(background class imbalance, focal loss)
